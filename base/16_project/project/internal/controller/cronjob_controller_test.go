...
create the CronJob.
*/
var _ = Describe("CronJob controller", func() { // 定义一类测试名称为 CronJob controller
	...
	Context("When updating CronJob Status", func() { // 细分测试类别 当CronJob status 更新的时候
		It("Should increase CronJob Status.Active count when new Jobs are created", func() { // 具体测试名称
			By("By creating a new CronJob") // 说明
			ctx := context.Background()
			cronJob := &cronjobv1.CronJob{ // 定义了一个CronJob
			...
			Expect(k8sClient.Create(ctx, cronJob)).Should(Succeed()) // 创建CronJob	并且检查是否成功
			...
			*/
			...
			cronjobLookupKey := types.NamespacedName{Name: CronjobName, Namespace: CronjobNamespace} // 定义一个查找的key
			createdCronjob := &cronjobv1.CronJob{}                                                   // 定义一个查找的CronJob

			// We'll need to retry getting this newly created CronJob, given that creation may not immediately happen.
			Eventually(func() bool { // Eventually 会一直重复执行这个函数，直到返回true
				err := k8sClient.Get(ctx, cronjobLookupKey, createdCronjob)
				return err == nil
			}, timeout, interval).Should(BeTrue())
			// Let's make sure our Schedule string value was properly converted/handled.
			Expect(createdCronjob.Spec.Schedule).Should(Equal("1 * * * *")) // 检查创建的CronJob的Schedule字段是否正确
			/*
			...
			*/
			...
			Consistently(func() (int, error) { // Consistently 在duration期间内会一直重复执行这个函数返回值始终为真。
				err := k8sClient.Get(ctx, cronjobLookupKey, createdCronjob)
				if err != nil {
					return -1, err
				}
				return len(createdCronjob.Status.Active), nil // cronJob 是定时器 只有在时间到了才会触发。 这里检查下没有执行的job.
			}, duration, interval).Should(Equal(0)) // 在duration时间内，每隔interval时间检查一次，检查是否有0个active job
			/*
			...
			*/
			...
			kind := reflect.TypeOf(cronjobv1.CronJob{}).Name() // get kind
			gvk := cronjobv1.GroupVersion.WithKind(kind)       // get gvk

			controllerRef := metav1.NewControllerRef(createdCronjob, gvk) // 创建一个controllerRef
